...
#define KEY_WEB_OTA_STATE "web_ota_state"
#define KEY_WEB_OTA_MSG "web_ota_msg"
...
String httpFallbackUrl(const String& url) {
    if (!url.startsWith("https://")) {
        return "";
    }
    return "http://" + url.substring(strlen("https://"));
}
...
int compareFirmwareVersions(const String& a, const String& b) {
    ...
    while (li < left.length() || ri < right.length()) {
        long lv = 0;
        long rv = 0;
        while (li < left.length() && left[li] != '.') {
            if (isDigit(left[li])) lv = lv * 10 + (left[li] - '0');
            ++li;
        }
        while (ri < right.length() && right[ri] != '.') {
            if (isDigit(right[ri])) rv = rv * 10 + (right[ri] - '0');
            ++ri;
        }
...
IotFeagri::IotFeagri(const char* user_default) : _userId(user_default), _mqttClient(_espClient) {
    ...
    _otaTaskNeedsStagger = false;
    _bootStatusPublished = false;
    _webOtaWritten = 0;
...
void IotFeagri::persistPendingFirmwareStatus(const String& version) {
    ...
    prefs.putString(KEY_FW_STATUS_MSG, version);
    prefs.end();
}

void IotFeagri::persistWebOtaStatus(const char* state, const String& message) {
    Preferences prefs;
    prefs.begin(PREF_NAME, false);
    prefs.putString(KEY_WEB_OTA_STATE, state ? state : "");
    prefs.putString(KEY_WEB_OTA_MSG, message);
    prefs.end();
}
...
bool IotFeagri::publishWebOtaLastStatus() {
    Preferences prefs;
    prefs.begin(PREF_NAME, true);
    String state = prefs.isKey(KEY_WEB_OTA_STATE)
                       ? prefs.getString(KEY_WEB_OTA_STATE)
                       : "";
    String message = prefs.isKey(KEY_WEB_OTA_MSG)
                         ? prefs.getString(KEY_WEB_OTA_MSG)
                         : "";
    prefs.end();
    ...
    if (publishFwStatus("web_ota_last", out.c_str())) {
        clearWebOtaStatus();
        return true;
    }
    return false;
...
void IotFeagri::clearWebOtaStatus() {
    Preferences prefs;
    prefs.begin(PREF_NAME, false);
    prefs.remove(KEY_WEB_OTA_STATE);
    prefs.remove(KEY_WEB_OTA_MSG);
    prefs.end();
}
...
bool IotFeagri::connectMQTT() {
    ...
    if (_mqttClient.connect(_deviceId.c_str(), _mqttUser.c_str(), _mqttPass.c_str())) {
        ...
        if (!_bootStatusPublished) {
            publishPendingFirmwareStatus();
            publishFwStatus("boot", "online");
            publishWebOtaLastStatus();
            _bootStatusPublished = true;
        }
        sendHeartbeat();
...
void IotFeagri::handleMqttMessage(char* topic, byte* payload, unsigned int length) {
    ...
    String ownerLower = lowerTrimmed(String(owner));
    String userLower = lowerTrimmed(_userId);

    if (ownerLower.length() > 0 && ownerLower != userLower) {
        return;
    }

    if (targetLower.length() > 0 && targetLower != clientLower &&
        !isGlobalTarget(targetLower.c_str()) && targetLower != groupLower &&
        targetLower != "grupo" && targetLower != "group") {
        return;
    }

    const char* group = doc["group"] | "";
    const bool collectiveTarget =
        targetLower.length() == 0 || isGlobalTarget(targetLower.c_str()) ||
        targetLower == groupLower || targetLower == "grupo" ||
        targetLower == "group" || isGroupTopic;
    if (collectiveTarget && !matchesGroup(group, _userId, groupId)) {
        return;
    }
    ...
    if (String(type) == "COMMAND" || String(type) == "CONFIG") {
        ...
        if (command == "UPDATE" || commandLower == "update_firmware" ||
            ...
            bool commandNeedsStagger =
                targetLower.length() == 0 || isGlobalTarget(targetLower.c_str()) ||
                targetLower == groupLower || targetLower == "grupo" ||
                targetLower == "group" || groupCommand == "todos" ||
                groupCommand == "all" || groupCommand == groupLower ||
                (isGroupTopic && targetLower != clientLower);
            publishCommandStatus("update_firmware", "starting",
                                 "Checking Manifest...");
            publishFwStatus("received", "command");
            if (!queueFirmwareUpdate(commandNeedsStagger)) {
                publishCommandStatus("update_firmware", "busy",
                                     "Update em andamento");
                publishFwStatus("error", "OTA ja pendente");
            }
...
bool IotFeagri::publishFwStatusDetail(const char* state, const String& message,
    ...
    if (progress >= 0 || written >= 0 || total >= 0 || url.length() > 0) {
        Serial.printf("[OTA] detail state=%s progress=%d written=%d total=%d url=%s\n",
                      state ? state : "", progress, written, total, url.c_str());
    }
    Serial.printf("[MQTT PUB] %s\n", userTopic.c_str());
...
bool IotFeagri::downloadFirmwareOnce(const String& fwUrl, const String& md5,
    ...
    if (!Update.begin(UPDATE_SIZE_UNKNOWN, U_FLASH)) {
        err = "Update.begin antecipado falhou: " + String(Update.errorString());
        return false;
    }
...
void IotFeagri::performUpdate(bool commandNeedsStagger) {
    ...
    String payload;
    int selected = -1;
    bool selectedViaHttpFallback = false;
    String lastErr;

    for (int i = 0; i < 3 && selected < 0; ++i) {
        String activeManifestUrls[2] = {
            manifestCandidates[i],
            httpFallbackUrl(manifestCandidates[i])
        };
        for (int urlIndex = 0; urlIndex < 2 && selected < 0; ++urlIndex) {
            if (activeManifestUrls[urlIndex].length() == 0) {
                continue;
            }
            if (urlIndex > 0) {
                publishFwStatusDetail("manifest_fallback", activeManifestUrls[urlIndex],
                                      -1, -1, -1, activeManifestUrls[urlIndex]);
            }
            publishFwStatusDetail("manifest_url", activeManifestUrls[urlIndex],
                                  -1, -1, -1, activeManifestUrls[urlIndex]);
            for (int attempt = 1; attempt <= 2; ++attempt) {
                ...
                if (!beginHttpRequest(http, activeManifestUrls[urlIndex], tcp, tls,
                ...
                if (httpCode == HTTP_CODE_OK) {
                    ...
                    selected = i;
                    selectedViaHttpFallback = urlIndex > 0;
                    http.end();
                    break;
                }
    ...
    if (selectedViaHttpFallback && urlUsesTls(fwUrl)) {
        String fallback = httpFallbackUrl(fwUrl);
        if (fallback.length() > 0) {
            fwUrl = fallback;
            publishFwStatusDetail("ota_url_rewrite", fwUrl, -1, -1, -1, fwUrl);
        }
    }
    if (urlUsesTls(fwUrl) && ESP.getMaxAllocHeap() < 60000) {
        String fallback = httpFallbackUrl(fwUrl);
        if (fallback.length() > 0) {
            fwUrl = fallback;
            publishFwStatusDetail("ota_url_rewrite", fwUrl, -1, -1, -1, fwUrl);
    ...
    if (manifestVersion.length() > 0 &&
        compareFirmwareVersions(manifestVersion, _fwVersion) <= 0) {
        publishFwStatus("skipped", "Firmware version already installed or not newer");
        _otaActive = false;
        return;
    }
    ...
    Serial.printf("[OTA] firmware_url=%s\n", fwUrl.c_str());
...
bool IotFeagri::queueFirmwareUpdate(bool commandNeedsStagger) {
    if (_otaActive || _otaTaskHandle != nullptr) {
        return false;
    }

    _otaTaskNeedsStagger = commandNeedsStagger;
    BaseType_t ok = xTaskCreatePinnedToCore(
        IotFeagri::otaTaskEntry, "iotfeagri_ota", 16384, this, 1,
        &_otaTaskHandle, 1);
    if (ok != pdPASS) {
        _otaTaskHandle = nullptr;
        return false;
...
void IotFeagri::otaTaskEntry(void* arg) {
    IotFeagri* self = static_cast<IotFeagri*>(arg);
    if (self != nullptr) {
        const bool needsStagger = self->_otaTaskNeedsStagger;
        self->performUpdate(needsStagger);
...
void IotFeagri::handleWebOtaUpload() {
    ...
    if (upload.status == UPLOAD_FILE_START) {
        ...
        _webOtaWritten = 0;
        _otaActive = true;
        persistWebOtaStatus("web_ota_start", upload.filename);
        publishFwStatus("web_ota_start", upload.filename.c_str());
        if (Update.isRunning()) {
            Update.abort();
        }
        if (!Update.begin(UPDATE_SIZE_UNKNOWN, U_FLASH)) {
            ...
            persistWebOtaStatus("web_ota_fail", _webOtaError);
            publishFwStatus("web_ota_fail", _webOtaError.c_str());
        }
        ...
        if (_webOtaError.length() > 0) {
            ...
            persistWebOtaStatus("web_ota_end", String(_webOtaWritten));
            publishFwStatus("web_ota_end", String(_webOtaWritten).c_str());
        }
        ...
        persistWebOtaStatus("web_ota_abort", _webOtaError);
        publishFwStatus("web_ota_abort", _webOtaError.c_str());
    }
}
...
void IotFeagri::handleWebOtaDone() {
    ...
    const String finalMsg = ok ? String("Upload concluido") : _webOtaError;
    persistWebOtaStatus(ok ? "web_ota_ok" : "web_ota_fail", finalMsg);
    publishFwStatus(ok ? "web_ota_ok" : "web_ota_fail",
                    finalMsg.c_str());
    delay(300);
